...
// `src/lib/connector-setup-pages.ts`). The generated files are consumed only
// where the hand-maintained map delegates to them (the mcp-client connector
// pilots the loader-map cutover); they are PARITY-CHECKED against the live
// scan + the hand-maintained map so they cannot drift. Full host consumption
// is the prototype slice + the cutover. `--check` is non-failing for now.
//
// Usage:

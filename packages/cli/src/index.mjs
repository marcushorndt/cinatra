...
function rebuildEnvLocal(repoRoot) {
  ...
  // Wayflow-container bridge auth shared secret. Without it the bridge 403s
  // every agent callback and `npm run services` (strict via gen-wayflow-env.mjs
  // --require-bridge-token) aborts; mint it like BETTER_AUTH_SECRET so a
  // rebuilt env is immediately runnable.

...
/**
 * Pre-fills inputParams so the execution setup loop (execution.ts:237 hasOwnProperty guard)
 * skips already-collected fields. Used by the Trigger page 'Run now' button.
 */
export async function createAndTriggerRunWithParams(
  args: CreatePendingRunArgs & { prefillInputParams: Record<string, unknown> },
): Promise<CreatePendingRunResult> {
  const session = await requireAuthSession().catch(() => null);
  const userId = session?.user?.id ?? null;
  if (!userId) return { ok: false, error: "unauthorized" };
  ...
  const created = await createAgentRunPendingInput({
    templateId: template.id,
    runBy: userId,
    inputParams: args.prefillInputParams,
    orgId,
  });
  ...
  try {
    ...
    if (err instanceof RunTransitionError && err.code === "stale_from_status") {
      return { ok: false, error: "state transition failed" };
    }
    throw err;
  }
  ...
  try {
    ...
    await transitionRunStatus(
      ...
      if (err instanceof RunTransitionError && err.code === "stale_from_status") {
        console.warn(
          `[createAndTriggerRunWithParams] compensation skipped for ${created.id}: run already advanced past queued`,
        );
        return;
      }
      console.error(
        "[createAndTriggerRunWithParams] compensation revert failed for run",
        created.id,
        err,
      );
      // Do not rethrow — the enqueue error is the user-facing error.
...
export type ScheduleAgentRunArgs = {
  templateSlug: string;
  prefillInputParams: Record<string, unknown>;
  scheduledAt: string; // ISO datetime string from client
};

/**
 * Creates a pre-filled run and enqueues it as a BullMQ delayed job. No DB migration
 * needed — delay is computed at enqueue time. Scheduled runs are snapshots; edits after
 * scheduling do not affect the queued run.
 */
export async function scheduleAgentRun(
  args: ScheduleAgentRunArgs,
): Promise<CreatePendingRunResult> {
  const session = await requireAuthSession().catch(() => null);
  const userId = session?.user?.id ?? null;
  if (!userId) return { ok: false, error: "unauthorized" };
  ...
  const delayMs = new Date(args.scheduledAt).getTime() - Date.now();
  if (delayMs < 0) return { ok: false, error: "scheduled time is in the past" };
  ...
  // Atomically transition pending_input → queued then enqueue with delay.
  try {
    await transitionRunStatus(created.id, "pending_input", "queued");
  } catch (err) {
    if (err instanceof RunTransitionError && err.code === "stale_from_status") {
  ...
  try {
    await enqueueAgentRun(
      { runId: created.id },
      { jobId: created.id, delay: delayMs },
    );
  } catch {
    // Revert to pending_input so the user can retry.
    // Discriminate the compensation catch so illegal_transition (programmer
    // error) surfaces loudly while stale_from_status (benign race — worker
    // already advanced the row) is logged and tolerated.
    await transitionRunStatus(
      ...
      if (err instanceof RunTransitionError && err.code === "stale_from_status") {
        console.warn(
          `[scheduleAgentRun] compensation skipped for ${created.id}: run already advanced past queued`,
        );
        return;
      }
      console.error(
        "[scheduleAgentRun] compensation revert failed for run",
        created.id,
        err,
      );
      // Do not rethrow — the enqueue error is the user-facing error.
